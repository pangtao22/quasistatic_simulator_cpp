...
CalcDfDuNumerical(QuasistaticSimulator *q_sim,
  ...
  for (int i_u = 0; i_u < n_u; i_u++) {
    ...
    auto x_next_plus = q_sim->CalcDynamics(x_nominal, u_plus, sim_params);
    auto x_next_minus = q_sim->CalcDynamics(x_nominal, u_minus, sim_params);
...
Eigen::MatrixXd
CalcDfDxNumerical(QuasistaticSimulator *q_sim,
                  const Eigen::Ref<const Eigen::VectorXd> &x_nominal,
                  const Eigen::Ref<const Eigen::VectorXd> &u_nominal,
  ...
  for (int i = 0; i < n_x; i++) {
    ...
    auto x_next_plus = q_sim->CalcDynamics(x_plus, u_nominal, sim_params);
    auto x_next_minus = q_sim->CalcDynamics(x_minus, u_nominal, sim_params);
