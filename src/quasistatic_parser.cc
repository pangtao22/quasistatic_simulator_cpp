...
template <typename T>
void SetValue(T &value_to_be_set, const YAML::Node &value) {
...
QuasistaticParser::QuasistaticParser(const std::string &q_model_path) {
  ...
  for (const auto &item : config["quasistatic_sim_params"]) {
    ...
    if (name == "h") {
      SetValue(sim_params_.h, value);
    } else if (name == "gravity") {
      auto &g_vec = value;
      sim_params_.gravity = Eigen::Vector3d(
          g_vec[0].as<double>(), g_vec[1].as<double>(), g_vec[2].as<double>());
    } else if (name == "nd_per_contact") {
      SetValue(sim_params_.nd_per_contact, value);
    } else if (name == "contact_detection_tolerance") {
      SetValue(sim_params_.contact_detection_tolerance, value);
    } else if (name == "is_quasi_dynamic") {
      SetValue(sim_params_.is_quasi_dynamic, value);
    } else if (name == "log_barrier_weight") {
      SetValue(sim_params_.log_barrier_weight, value);
    } else if (name == "unactuated_mass_scale") {
      SetValue(sim_params_.unactuated_mass_scale, value);
    } else {
      std::stringstream ss;
